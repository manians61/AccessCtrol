...
namespace AccessControl.API.Repositories
{
    ...
    {
        public PermissionRepository(IDbConnectionFactory dbConnectionFactory) : base(dbConnectionFactory, 1)
        { }
        public async Task<IEnumerable<UserAccessDto>> GetAllPermissions(string user_ID)
        {
            var query = @"SELECT * FROM dbo.User_Permission_View WHERE User_ID = @user_ID";
            try
            {
        ...
        {
            var query = @"SELECT DISTINCT Group_Name, Group_ID FROM dbo.User_Permission_View WHERE User_ID = @user_ID";
            try
            {
        ...
        {
            var query = @"SELECT DISTINCT PermissionObj_Name FROM dbo.User_Permission_View WHERE User_ID = @user_ID AND Group_Name = @group_ID";
            try
            {
