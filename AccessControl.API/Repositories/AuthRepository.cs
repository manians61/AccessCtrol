        ...
        private SqlConnection GetConnection(int login_Type)
        {
            ...
            switch (login_Type)
            {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    conStr = _iConfig.GetSection("ConnectionStrings").GetSection("PrdConnection").Value;
                    break;
            }
        ...
        public async Task<bool> UserExists(string user_ID)
        {
            using (SqlConnection DbConnection = GetConnection(1))
            {
                try
                {
                    ...
                    ("SELECT COUNT(1) FROM dbo.Report_Hub_User WHERE User_ID = @user_ID", new { @user_ID = user_ID });
                }
                catch (Exception ex)
                {
                    throw ex;
